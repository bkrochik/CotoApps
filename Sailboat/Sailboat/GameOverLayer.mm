...
-(id) init
{
    if( (self=[super init])) {
        ...
        statusLabel.position = ccp(winSize.width/2, (winSize.height/2)+(80));
        ...
        [myMenu setPosition:ccp(winSize.width/2, (winSize.height/2)-(30))];
...
- (void)resetAction:(id)sender
{
    ...
    [[CCDirector sharedDirector] replaceScene:
     [CCTransitionFlipY transitionWithDuration:1.8f scene:[WorldLayer scene]]];
}
-(void) dealloc
{
	delete world;
